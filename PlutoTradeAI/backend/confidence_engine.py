def calculate_confidence(stock):
    ...
    if stock["distance_to_resistance_percent"] <= 5:
        confidence += 10
        reasons.append("Price is near resistance/breakout zone")
...
def suggest_action(confidence, stock):
    if confidence >= 80:
        return "High-confidence setup — review for E*TRADE later"

    if confidence >= 65:
        if stock["on_watchlist"]:
            return "Strong setup on watchlist — review trade"

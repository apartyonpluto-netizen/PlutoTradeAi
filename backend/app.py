...
# See webull_api.preview_stop_and_target_combo's own comment for the full
# context - a real SLB entry's take-profit leg was rejected as a "reverse
# position" because the stop and target legs are placed as two independent
# orders. This calls Webull's genuine dry-run preview_order endpoint (never
# executes, never touches capital) against the REAL, currently-stuck entry's
# own stored stop/target prices and the broker's own current position
# quantity, to find out - with real evidence, not an assumption - whether a
# true STOP_LOSS_PROFIT combo order is accepted by this sandbox account.
# Remove once STOP_LOSS_PROFIT combo orders are either adopted for real or
# ruled out.
@app.route("/api/admin/diagnostic/preview-stop-target-combo", methods=["POST"])
def api_admin_diagnostic_preview_stop_target_combo():
    guard = _require_admin()
    if guard:
        return guard
    payload = request.get_json(silent=True) or {}
    ...
    if matching_order is None:
        return _api_failure(f"No overnight order found for {ticker} on this account.", status_code=404, error_code="not_found", ok=False)
    ...
    try:
        ...
        return _api_failure(f"Could not fetch current position: {error}", status_code=502, error_code="broker_error", ok=False)
    live_quantity = next(
        (float(position.get("quantity", 0) or 0) for position in positions if str(position.get("symbol", "")).upper() == ticker),
        None,
    )
    if not live_quantity:
        return _api_failure(f"No live open position found for {ticker} to preview a combo against.", status_code=404, error_code="not_found", ok=False)

    try:
        preview = webull_api.preview_stop_and_target_combo(
    ...
    return _api_success(
        {"live_quantity": live_quantity, "stop_price": matching_order.get("stop"), "target_price": matching_order.get("target"), "preview": preview},
        ok=True,
    )
